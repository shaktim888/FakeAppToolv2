    ...
    this.exportApp = function (jsondata, workpath) {
        ...
        for (var key in jsondata) {
            ...
            var hallViewStyle = define.GAME_STYLE_TABLE[config.PACKAGE_GAME_STYLE].hall_layout
             if (hallViewStyle && define.UsePokerLayoutList()){
                global.ISL = define.isL()
                let isL = ISL == true ? "heng" : "shu"
                if (key == "common_res"){
                    workpath = `../${hallViewStyle}/GameRes/common/`
                }
                else if (key == "game_layer"){
                   
                }
                else if (key == "Special"){
                    workpath = `../${hallViewStyle}/Special`
                    key = ""
                }
                else if (key == "setting"){
                    workpath = `../${hallViewStyle}/GameRes/common/setting`
                    ...
                    confData.forEach(ele =>{
                        if (key == ele.component){
                            if(ele.parent_directory){
                                workpath = `../${hallViewStyle}/GameRes/${ele.parent_directory}/${key}`
                            }
                            else{
                                workpath = `../${hallViewStyle}/GameRes/${isL}/${key}`
                            }
                        }
            ...
            this.handleModules(eleData,path.join(workpath, key), IS_ARCHIVE, key)
        }
    }
    this.handleModules = function (moduledata, workpath, IS_ARCHIVE,moduleName) {
        for (var key in moduledata) {
            if(moduleName == "Special"){
                let Conf = `../${hallViewStyle}/Special/map.json`
                let confData = JSON.parse(fs.readFileSync(Conf));
                confData.forEach(ele =>{
                    if(ele.parent_directory){
                        path.join(workpath, ele.parent_directory)
                    }
                    else{
                        let game_isL =  (define.isL()) == true ? "heng" : "shu"
                        path.join(workpath, game_isL)
                    }
                })
            }
